...
def parse_args():
    p = argparse.ArgumentParser(description="Compute stock returns (daily/weekly/monthly/quarterly/yearly) from tw_stock_prices to tw_stock_returns")
    g_scope = p.add_mutually_exclusive_group(required=False)
    g_scope.add_argument("--all", action="store_true", help="Process all symbols in tw_stock_prices")
    ...
    p.add_argument("--fill-missing", action="store_true", help="Only compute rows not existing in tw_stock_returns")
    args = p.parse_args()
...
def process_symbol(symbol: str, start: str | None, end: str | None, fill_missing: bool):
    ...
    # If fill-missing: filter out dates that already exist in tw_stock_returns
    if fill_missing:
...
def main():
    ...
    console.print(f"[green]Done. Upserted {total_written} rows into tw_stock_returns.[/green]")

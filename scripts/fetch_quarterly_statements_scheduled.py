...
def run(args: argparse.Namespace) -> int:
    auto_year, auto_season = resolve_quarter()
    year = args.year or auto_year
    season = args.season or auto_season
    if not 2000 <= year <= 2100:
        raise ValueError("year 必須介於 2000 到 2100")
    if season not in (1, 2, 3, 4):
        raise ValueError("season 必須為 1 到 4")
    if bool(args.year) != bool(args.season):
    ...
    logger.info("=" * 72)
    logger.info("季度三大報表抓取：%d Q%d", year, season)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--year", type=int, help="西元年度；省略時自動判斷最新公告季度")
